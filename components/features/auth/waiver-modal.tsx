...
export function WaiverModal({ isOpen, onOpenChange, onSigned }: WaiverModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIsAgreed(false)
      // Don't reset name/address if they typed it before closing by accident
    }
  }, [isOpen])

  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col bg-zinc-950 border-zinc-800 text-white p-0 gap-0 overflow-hidden sm:rounded-2xl">
        {/* Header */}
        <div className="p-6 border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogTitle className="text-2xl font-bold text-center font-heading tracking-tight">
              Release of Liability & Waiver
            </DialogTitle>
            <DialogDescription className="text-center text-zinc-400 text-base max-w-md mx-auto mt-2">
        ...
        {/* Scrollable Content */}
        <div
          className="flex-1 overflow-y-auto p-6 bg-zinc-950 relative group"
